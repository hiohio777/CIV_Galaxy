...
public class PlayerCivInfo : MonoBehaviour
{
    ...
    public void Show(ICivilization civ)
    {
        ...
        //countFromPlanet.text = $"<color=#ffff00ff>+{Math.Round(civ.CivData.GetPointsFromPlanets, 1)}</color> <color=#008000ff>rate: {Math.Round(civ.DataBase.Base.GDPlanets + civ.CivData.GDPlanets, 1)}</color>";
        //countFromIndustry.text = $"<color=#ffff00ff>+{Math.Round(civ.CivData.GetPointsFromIndustry, 1)}</color> <color=#008000ff>rate: {Math.Round(civ.DataBase.Base.GDIndustry + civ.CivData.GDIndustry, 1)}</color>";
       //countFromBonus.text = $"<color=#ffff00ff>+{Math.Round(civ.CivData.GetPointsFromBonus, 1)}</color> <color=#008000ff>rate: {Math.Round(civ.DataBase.Base.GDOverall + civ.CivData.GDOverall, 1)}</color>";
